...
import edu.stevens.cs548.clinic.dto.RadiologyType;
...
public class App {
	...
	private long addProvider(String[] args) throws ServiceError {
		if (args.length != 3) {
			error("Usage: provider addProvider name  provider-id specialization");
		}
		try {
	...
	private ProviderDto getProvider(String[] args) throws ServiceError {
		...
		try {
			return provider.getProviderByProId(providerKey);
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		}
	...
	private ProviderDto getProviderByProId(String[] args) throws ServiceError {
		if (args.length != 1) {
			error("Usage: provider getProviderByProId provider-id");
		}
		long providerId = Long.parseLong(args[0]);
		logger.info("Getting a provider by provider id (" + providerId + ").");
		try {
	...
	private long addDrugTreatment(String[] args) throws ServiceError {
		...
		long treatmentId = 0;
		try {
			treatmentId = provider.addTreatment(dto);
			logger.info("Added treatment with id : " + treatmentId);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return treatmentId;
	...
	private long addRadiologyTreatment(String[] args) throws DatatypeConfigurationException {
		if (args.length < 4) {
			error("Usage: provider addRadiologyTreatment patient-key provider-key diagnosis date(s)");
		}
		TreatmentDto dto = factory.createTreatmentDto();
		dto.setPatient(Long.parseLong(args[0]));
		dto.setProvider(Long.parseLong(args[1]));
		...
		for (int i = 3; i < args.length; i++) {
			...
			GregorianCalendar gcal = new GregorianCalendar();
			gcal.setTime(dob.getTime());
			rt.getDate().add(DatatypeFactory.newInstance().newXMLGregorianCalendar(gcal));
		}
		dto.setRadiology(rt);
		long treatmentId = 0;
		try {
	...
	private long addSurgeryTreatment(String[] args) throws DatatypeConfigurationException {
		if (args.length != 4) {
			error("Usage: provider addSurgeryTreatment patient-key provider-key diagnosis date");
		}
		TreatmentDto dto = factory.createTreatmentDto();
		dto.setPatient(Long.parseLong(args[0]));
		dto.setProvider(Long.parseLong(args[1]));
